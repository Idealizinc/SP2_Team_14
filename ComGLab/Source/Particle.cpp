...
void Particle::renderParticleEmitter(GLuint Texture, Vector3 Direction, Vector3 Velocity, int ParticleCount, int Spread)
{
	...
	RenderMesh(meshList[GEO_CUBE], true);
		modelStack.PushMatrix();
		//Render Particles
		Mesh* particleQuad = meshList[GEO_QUAD];
		particleQuad->textureID = Texture;
		RenderMesh(particleQuad, false);
		//Update the particles in accordance

		//Sort the particles based on position
		modelStack.PopMatrix();
	modelStack.PopMatrix();
}
